...
class Marca(models.Model):
    fabricante = models.ForeignKey(Fabricante, on_delete=models.CASCADE, related_name='marcas')
    nome = models.CharField(max_length=100)
...
class Produto(models.Model):
    ...
    num_decimais = models.PositiveIntegerField("Número do decimais", null=False, default=0,
                                               ...
                                               help_text='Máscara de Número de decimais a utilizar durante as '
                                                         'operações')
    # exemplo de uso: 200 mL
    quantidade = models.CharField(max_length=20, blank=True, null=True)
    ...
    o_que_e = models.CharField(max_length=500, blank=True, null=True)
    para_que_serve = models.CharField(max_length=500, blank=True, null=True)
    modo_de_usar = models.CharField(max_length=500, blank=True, null=True)
    
    ean = models.CharField(max_length=500, blank=True, null=True)
    
    bula = models.CharField(max_length=100, blank=True, null=True)
    registro_ms = models.CharField(max_length=50, blank=True, null=True)
    ...
    fator = models.CharField(max_length=100, blank=True, null=True)
    ...
    def get_bula(self):
        if self.bula is None:
            return 'Não'
        else:
            return self.bula
        
    def get_registro_ms(self):
        if self.registro_ms is None:
            return 'Não'
        else:
            return self.registro_ms

    def get_principio_ativo(self):
        if self.principio_ativo is None:
            return 'Não'
        else:
            return self.principio_ativo

    def get_dosagem(self):
        if self.dosagem is None:
            return 'Não'
        else:
            return self.dosagem
...
class PedidoWeb(models.Model):
    ...
    status_pedido = models.CharField("Status deste pedido", max_length=1, null=False, default="w",
                                     choices=STATUS_PEDIDO_ITEM_CHOICES,
                                     help_text='status do pedido Orçamento/Devolução/Cancelado/Em análise crédito/'
                                               'Bloqueado/Faturado/quitado/Pré-Pedido etc ')
    ...
    def total_preco_unitario(self):
        return str(sum(pedidowebitem.tot_preco_unitario() for pedidowebitem in self.items.all()))

    def total_desconto(self):
        return str(sum(
                        pedidowebitem.valor_desconto() for pedidowebitem in self.items.all()
                        )
                   )

    def maior_sequencia(self):
        return str(max(pedidowebitem.sequencia for pedidowebitem in self.items.all()))

    def total_preco_bruto(self):
        return str(sum(pedidowebitem.total_preco_bruto() for pedidowebitem in self.items.all()))
    
    def total_preco_liquido(self):
        return str(sum(
                        pedidowebitem.total_preco_bruto() for pedidowebitem in self.items.all()) -
                   sum(pedidowebitem.valor_desconto() for pedidowebitem in self.items.all())
                  )
...
class PedidoWebItem(models.Model):
    pedidoweb = models.ForeignKey(PedidoWeb, related_name='items', on_delete=models.CASCADE)
    sequencia = models.PositiveIntegerField("Sequência do item", null=False, default=1)
    produto = models.ForeignKey(Produto, on_delete=models.CASCADE)
    ...
    quantidade = models.DecimalField("Quantidade", max_length=16, max_digits=16, decimal_places=6,
                                     default=1, validators=[MinValueValidator(1.0)],
                                     help_text='Quantidade vendida de produtos no pedido e nota fiscal de entrada ou '
                                               'de saida')
...
@receiver(post_delete, sender=PedidoWebItem)
def renumera_itens(sender, **kwargs):
    """
        Essa função renumera os itens restantes de um pedido, depois que um item é apagado.
     
    """
    for idx, p in enumerate(PedidoWebItem.objects.filter(pedidoweb_id=kwargs['instance'].pedidoweb_id)):
        p.sequencia = idx + 1
        p.save()

        
@receiver(post_delete, sender=PedidoWebItem)
@receiver(post_save, sender=PedidoWebItem)
def atualiza_pedido(sender, **kwargs):
    """
        Essa função atualiza o valor total de um pedido, toda a vez que item for adicionado ou apagado.
    """
    p = PedidoWeb.objects.get(id=kwargs['instance'].pedidoweb_id)
    p.total_produtos = p.total_preco_liquido()
    p.save()
